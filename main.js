"use strict";
(() => {
    //  console.log("LiveServer Plugin is initializing...");
    // lets initilixe the modules so thatl well avble to tonuse them
    ...
    class LiveServer {
        ...
        constructor () {
             this.port = null;
            /// ill use it in next update
            this.reloadFile = () => {
                let iframe = document.getElementById('iframe');
                //     //   iframe.contentWindow.location.reload();
                //     //iframe.src = `http://localhost:3000/preview/${jsonData.fileName}`;
                //     iframe.src = 'http://localhost:3000/';
                //     //  location.reload();///)this will relode entirr acode
        ...
        async init() {
            ...
            this.openWindow = () => {
                ...
                if (!document.getElementById("live-server-window")) {
                    ...
                    //  console.log('window alredy exists')
                }
            }
            ...
            function showWindow() {
                //   console.log('called show window')
                settings.update({
                    autosave: 1000
                });
                //  console.log("Appending window to body...");
                if (!document.body) {
            ...
            function handleTheBackend() {
                ...
                if (!savedFilePath && !cacheFilePath) {
                    //  console.log('rawPath and cacheFilePath both arent defined');
                    return;
                } else {
                    rawPath = savedFilePath || cacheFilePath; //it will assign a value whis is avilable
                    if (savedFilePath) {
                        //  console.log('the opend file is saved file')
                        ...
                        if (!originalPath) {
                            // console.log('cannot resolve path');
                            return;
                        } else {
                        ...
                        ///     console.log('the opend file is a cache file')
            ...
            //   this.showOrHideIFhtml = showOrHideIFhtml;
            ...
            async function checkServer(jsonData) {
                ...
                try {
                    ...
                    setTimeout(() => {
                        ...
                        //     iframe.src = `http://localhost:3000/preview/${jsonData.fileName}`;
                        //  iframe.src = 'http://localhost:3000/';
                        //   iframe.reload(); // thers no method callde .relode()
                        // whenever src changes then its autometically reloads tye window
                        // >>>> or if in future its needed to force it ill use this
                        // iframe.contentWindow.location.reload(); // only if same-origin
                        if (iframe) {
        ...
        }//this is cloasing of init function
    ...
    //   console.log("Checking Acode instance:", window.acode);
